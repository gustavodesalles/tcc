...
A primeira etapa consiste em um levantamento teórico sobre os conceitos, ferramentas e práticas envolvidas na orquestração de \textit{pipelines} de ETL (\textit{extraction-transformation-loading}) utilizando o Apache Airflow. Foram reunidos artigos científicos, publicações técnicas e documentação especializada sobre a estrutura e implementação de DAGs (\textit{directed acyclic graphs}) no Airflow, bem como sobre métodos de \textit{web scraping}, a fim de possibilitar a extração de dados não estruturados diretamente de páginas da Web.

Na segunda etapa, foi realizada a implementação dos \textit{scripts} responsáveis pela extração de dados por meio de \textit{web scraping} e requisições a APIs REST. Os dados obtidos a partir dos \textit{endpoints} públicos disponibilizados no Portal de Transparência da Prefeitura estavam incompletos, o que tornou necessário consultar os \textit{endpoints} internos do sistema para extrair os dados faltantes. Após este processo, realizou-se uma análise exploratória dos dados, consistindo na geração de gráficos para visualização de possíveis \textit{outliers} entre os registros e na construção de um dicionário de dados e, subsequentemente, a modelagem de um esquema para bancos de dados relacionais.
...
Esta seção é dividida em duas subseções. A subseção 2.4.1 descreve a API de compras públicas disponível no \textit{website} do Portal de Transparência da Prefeitura de Florianópolis, o método de consulta de dados e os parâmetros de pesquisa. A subseção 2.4.2 apresenta um dicionário de dados, no qual os conteúdos das respostas da API são esquematizados em tabelas.

\subsection{API de compras públicas da PMF}
...
A API possui diversos \textit{endpoints}, dos quais são utilizados os de consulta de licitações\footnote{Disponível em: \licitacoes} e de contratos administrativos\footnote{Disponível em: \contratos}. O acesso a estes \textit{endpoints} é livre, sem necessidade de chave para autenticação.

O único método fornecido para ambos os \textit{endpoints} é o GET, que retorna dados de licitações ou contratos em formato JSON. Em ambos os \textit{endpoints}, é possível filtrar os resultados pelas datas inicial e final do período de busca, o código da unidade gestora responsável e o limite máximo de registros a serem retornados. Apenas as datas inicial e final são parâmetros obrigatórios.
...
\subsection{Descrição dos dados retornados}
...
\begin{figure}[htb]
    ...
    \caption{Visão em árvore do JSON de licitações.}
...
O \textit{Website} do Portal de Transparência da Prefeitura de Florianópolis também disponibiliza \textit{endpoints} para a consulta de receitas, despesas orçamentárias, execuções de despesa e dados da gestão de pessoal; estes não foram utilizados no trabalho, devido aos dados possuírem pouca relação ao contexto dos processos licitatórios.

Os arquivos JSON seguem padrões semelhantes; ambos os \textit{endpoints} consultados no trabalho retornam arquivos com as seguintes propriedades:
...
\hline
informacao & Texto & 50 & Tipo de informação retornada pela API (\texttt{`Licitacao'}, \texttt{`Contrato'}, \texttt{`Receita'},  \texttt{`Despesa Orçamentária'}, \texttt{`Execução de Despesa'}). \\
\hline
...
% \begin{itemize}
%     \item "licitacao": dados principais da licitação, como número identificador, modalidade, valor estimado, resumo do objeto, datas de emissão e abertura, finalidade e forma de julgamento.
%     \begin{itemize}
%         \item "numero";
%         \item "modalidade";
%         \item "valorEstimado";
...
%         \item "aberturaData";
%         \item "finalidade";
%         \item "formaJulgamento";
%     \end{itemize}
%     \item "unidadeGestora": identificação do órgão responsável pela licitação.
%     \item "advogado": nome do representante jurídico vinculado ao processo licitatório.
%     \item "listItens": itens licitados, com descrição, quantidade, unidade, valor estimado e situação.
%     \begin{itemize}
%         \item "numero";
%         \item "denominacao";
%         \item "quantidade";
...
%         \item "situacao";
%         \item "listVencedores": para cada item, a lista de fornecedores vencedores, com valores e quantidades ofertadas.
%         \begin{itemize}
%             \item "fornecedor";
%             \item "quantidade";
%             \item "valorUnitario";
...
%         \end{itemize}
%     \end{itemize}
%     \item "listTextos": documentos e arquivos complementares (ex.: edital, ata, contratos), com link para acesso.
%     \item "listEmpenhos" (opcional): informações sobre os empenhos associados à licitação, como data de emissão, número, espécie, categoria e objeto do empenho.
%     \begin{itemize}
%         \item "emissao";
%         \item "numero";
%         \item "objetoResumido";
...
%         \item "recursoDiaria";
%     \end{itemize}
%     \item "listContratos" (opcional): informações de contratos associados à licitação, como número, valor total e datas de assinatura e início de vigência.
% \end{itemize}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.4\linewidth]{images/json_contrato_v2.png}
    \caption{Visão em árvore do JSON de contratos.}
...
% \begin{itemize}
%     \item "contrato": dados principais do contrato, como número identificador, datas de assinatura, início de vigência e vencimento, valor total e resumo do objeto.
%     \begin{itemize}
%         \item "numero";
%         \item "assinatura";
%         \item "inicioVigencia";
...
%         \item "objetoResumido";
%     \end{itemize}
%     \item "unidadeGestora": identificação do órgão responsável pelo contrato.
%     \item "fornecedor": nome da pessoa ou empresa contratada.
%     \item "listItens": descrição dos itens contratados, com denominação, quantidade, unidade de medida e valores unitário e total.
%     \begin{itemize}
%         \item "numero";
%         \item "denominacao";
%         \item "quantidade";
...
%         \item "valorTotal";
%     \end{itemize}
%     \item "listEmpenhos" (opcional): informações sobre os empenhos associados ao contrato, como data de emissão, número, espécie, categoria e objeto do empenho.
% \end{itemize}
...
Este capítulo descreve o que foi desenvolvido no trabalho de conclusão de curso, incluindo codificação do cliente da API, análise exploratória de dados coletados, definição do esquema do banco de dados destino implementação do processo de ETL. 
...
\begin{center}
	...
	\includegraphics[trim={17cm 0 0 0},clip,width=\textwidth]{images/modelo_logico_tcc_v3.png} %scale=0.32
	\fonte{Elaborada pelo autor.}
\end{center}
\end{figure}

Além das tabelas definidas no dicionário de dados, foi criada uma tabela \texttt{ItemFornecedor} para modelar o relacionamento entre as tabelas \texttt{Item} e \texttt{Fornecedor}, considerando que um item de licitação ou contrato pode possuir mais de um fornecedor.
...
Esta seção apresenta o planejamento e implementação do processo de extração, transformação e carga dos dados obtidos nas seções anteriores. As etapas do processo de ETL serão determinadas após a conclusão da modelagem do esquema relacional dos dados.
...
Em geral, nota-se uma grande quantidade de registros de licitações cuja unidade gestora é a Prefeitura Municipal de Florianópolis (código 1) ou o Instituto de Previdência Social dos Servidores Públicos (código 22) que não possuem valores estimados informados. Isto resulta em um \textit{boxplot} com muitos \textit{outliers}, como exemplificado pela \autoref{fig:boxplot_licitacoes}.
...
As APIs da Prefeitura Municipal de Florianópolis oferecem dados incompletos entre si, sendo necessário realizar requisições em diversos \textit{endpoints} para obter informações completas. Para trabalhos futuros, é sugerido o desenvolvimento de uma única API que reúna todos os conteúdos em um arquivo.
